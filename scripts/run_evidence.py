...
STABILITY_RUNS = 20
"""Only used to describe the stability folder in the generated README;
`scripts/run_stability.py` owns the real default.
"""
...
def render_readme(rows: list[dict]) -> str:
    """The whole file, generated -- not a hand-written page with a
    generated table dropped into it.

    The "actual" and "exit code" columns are the point of this document,
    and they are only worth reading if they cannot have been typed by
    someone who believed them. Generating the surrounding prose too means
    there is no hand-maintained half to drift out of step when a scenario
    is added or an outcome changes.
    """
    header = (
    ...
    for row in rows:
        ...
        if actual != row["expected"]:
            actual = f"**{actual}** :warning:"
        extras = " + `failure.png`" if row["screenshot"] else ""
        lines.append(
            f"| `{row['scenario']}` | {row['brief']} | `{row['expected']}` | {actual} "
            f"| `{row['exit_code']}` | [`{row['folder']}/`]({row['folder']}/){extras} |"
        )
    table = header + "\n".join(lines)

    in_process = sorted(r["scenario"] for r in rows if r["mode"] == "in-process")
    disagreeing = [r for r in rows if r["actual"] != r["expected"]]
    verdict = (
        "Every scenario ended where it was expected to."
        if not disagreeing
        else f"**{len(disagreeing)} scenario(s) did not end where expected** -- see the marked rows."
    )

    return f"""<!-- Generated by scripts/run_evidence.py. Do not edit by hand: the
...
status. {", ".join(f"`{n}`" for n in in_process)} run in-process instead,
...
| [`stability/`](stability/) | {STABILITY_RUNS} replays against injected faults, scored for **wrong answers** (values that aren't the requested member's) and for outcomes that didn't match their fault. Produced by `scripts/run_stability.py`. |
