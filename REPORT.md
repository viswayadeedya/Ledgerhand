...
opens*: `abc` can't become a correct answer however well the run goes, and
reported late it surfaces as whichever step fell over first. It is the
caller's contract, not the app's — an input that passes can still come back
`member_not_found`.
...
replay plus a business-outcome run; another asserts, in a fresh
interpreter, that importing the replay CLI pulls in no `anthropic` module,
with a control test asserting discovery *does* — without which the first
would pass if the SDK were deleted. Everything comes from `artifact.steps`,
...
`input_invalid`, …), so failures are triaged by kind rather than grepped:
`element_not_found` is drift, `identity_mismatch` a correctness emergency.
...
| Waiting bounded two ways: instant check, bounded poll | Instant for strict single-match semantics; poll where cross-frame navigation needs wall-clock time. Both were bugs first |
| UI drift: ranked fallback, then loud failure | Secondary to runtime errors per the brief. The failure names the failing candidate — the signal to re-record |
...
| "Still loading" separated from "not there" | Both arrive as "no element matched", need opposite responses. An in-flight document request catches the server still thinking; `readyState` the parse phase after |
...
**Limits.** *Status codes aren't universal*: many legacy apps answer
`200 OK` with the error in the body. Those need per-app *text* detection on
the artifact — the `detect` shape `business_outcomes` already has, plus an
equivalent on the failure side. Status stays the capability-independent
default; declared text would be the override. Not built, because the fake
app signals honestly and it would ship a mechanism with no real case behind
it. *The load check reads the document*: a page that returns fast and
fetches data in the background reports `complete` with nothing on it, so
that shape of slow load still reads as `element_not_found` — degrading
toward the old behaviour, not a wrong answer. A client-rendered surface
would need a pending-XHR count or a ready marker.
...
model unattended, in tests and evidence), `TerminalOperatorHandoff` (a real
person and a real decision; only "look at the page" is mocked).
...
`automation`) with a holder, a reason and both endpoints; a run nobody
touched is one uninterrupted span, because "nobody took over" and "we
didn't track it" must look different. Spans open and close around the
`escalate()` call, so the timeline records the fact the call stack already
...
which is the first thing someone taking over a stuck run does. Values are
masked on arrival (`***02`); a password never leaves the page at all, per
the Part 4 rule that a value a scan can reach is one someone forgets to
redact. A capture failure is recorded, so an empty list always means "they
did nothing".

**Its limit**: it observes the *document*, so what the operator does to the
browser — back/forward, address bar, new tabs, native dialogs — is chrome,
not DOM, and invisible. A `framenavigated` entry records where they ended
up, not how. The log is faithful about the page and incomplete about the
operator; completing it means browser-level events (CDP transition types,
plus the existing dialog handler feeding the same log).
...
redaction one.** A run log records what the model saw; a screenshot shows a
balance as plainly as any JSON, and no text scan catches it. Masking them
doesn't make them safer evidence, it stops them being evidence — what they
prove is that a model read a real page and reached a real answer. In
production they belong in access-controlled, short-retention storage, never
a repository, deleted once the artifact they produced is approved; the
artifact is the durable object and never holds those values either way.
Here `evidence/discovery-member-lookup/run_log.json` is exempt from the
values rule by exact path, with that reasoning in the test, and still
subject to the credential rule.

**Limits:** redaction is key-name and pattern based, so an unanticipated
secret-shaped field isn't caught; sensitivity is declared per artifact, so
an output nobody marked stays unmasked; screenshots aren't masked, because
they're pixels and pretending otherwise would be worse than saying so; the
allowlist is route-based, so a rename needs a policy update; there is no
secrets vault (credentials are passed at call time and held in memory for
the run) and no rate-limiting or anomaly detection beyond the allowlist.
...
| Business outcomes and the sub-account capability by deterministic capture, not LLM re-discovery | The required LLM run is the happy path. The sub-account flow *cannot* be discovered unattended — policy blocks its commit and discovery has no handoff path, so the run stalls at the step the artifact needs |
...
| Prose-only confirmation pages | An output appearing only inside a sentence gets a `text` locator pinned to one run's value. Needs a capture-group pattern on the output, not a locator — a real schema gap, named rather than papered over |
